...
- Prefer these package groups for user-level CLI tools that are just binaries.
  Examples: `shellcheck`, `fd`, `duf`, `glow`, `sops`, `age`, `awscli2`,
  `ansible`, `opentofu`, and `terraform-docs`.
...
- `cloud.nix` - AWS, Ansible, Kubernetes, OpenTofu, and infrastructure CLIs.
...
- `text.nix` - Document, Markdown, media, PDF, JSON, YAML, and search helpers.
